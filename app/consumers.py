...
from app.utils.consumer_functions import *
...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        await self.channel_layer.group_send(
            self.room_group_name,
            {
                ...
                "image_profile": (
                    self.user.image_profile.url
                    if self.user.image_profile
                    else "/static/global/images/image_default.png"
                ),
            },
        )
                ...
                text_data=json.dumps(
                    {
                        ...
                        "user_id": msg.user.id,
                        "image_profile": (
                            msg.user.image_profile.url
                            if msg.user.image_profile
                            else "/static/global/images/image_default.png"
                        ),
    ...
    async def send_online_count(self):
        ...
        await self.channel_layer.group_send(
            self.room_group_name, {"type": "online_count", "count": count}
        )

        await self.channel_layer.group_send(
            "lobby",
            {"type": "user_count_update", "room": self.room_name, "count": count},
        )
    ...
    async def receive(self, text_data):
        ...
        if data.get("type") == "typing":
            ...
            await self.channel_layer.group_send(
                self.room_group_name, {"type": "stop_typing", "user_id": self.user.id}
            )
            ...
            await database_sync_to_async(Message.objects.create)(
                user=self.user, room=room, content=message, timestamp=now()
            )
    ...
    async def stop_typing(self, event):
        await self.send(
            text_data=json.dumps({"type": "stop_typing", "user_id": event["user_id"]})
        )
    ...
    async def online_count(self, event):
        await self.send(
            text_data=json.dumps({"type": "online_count", "count": event["count"]})
        )
...
class GlobalPresenceConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        print(f"[SOCKET] Adicionando {self.user.username} globalmente como online")
        await add_global_user_online(self.user.id)
        await self.accept()

    async def disconnect(self, close_code):
        if self.user.is_authenticated:
            await remove_global_user_online(self.user.id)
            await set_last_seen(self.user.id)
